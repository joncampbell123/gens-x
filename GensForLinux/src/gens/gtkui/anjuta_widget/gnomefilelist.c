...
#define FILE_PIXMAP_FOLDER DATADIR"/bfoldc.xpm"
#define FILE_PIXMAP_FILE   DATADIR"/file_text.png"
...
gnome_filelist_new_with_path (const gchar * path)
{
	...
	gtk_button_box_set_spacing (GTK_BUTTON_BOX (util_box), /* GNOME_PAD */ 2);
	gtk_widget_show (util_box);
...
check_ok_button_cb (GtkWidget * widget, GnomeFileList * file_list)
{
	...
	if (file_list->dir_only)
	{
		if (gnome_filelist_check_dir_exists (path))
			gtk_widget_set_sensitive(file_list->ok_button, TRUE);
			return;
	}
...
home_directory_cb (GtkButton * button, GnomeFileList * file_list)
{
	...
	home = g_get_home_dir();// gnome_util_user_home ();
